...
Sorting itself is up to you - the classes only style the header. Tabler's own demo uses [DataTables.net](https://datatables.net) with Bootstrap 5 styling (see the `datatables` preview page and the `DataTable` component), which enhances the header automatically.
