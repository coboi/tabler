...
Section labels render only in vertical navbars — a horizontal navbar hides them.
...
The nav scrolls independently, so the footer stays visible with long menus. Below the navbar's breakpoint, the footer moves into the mobile top bar: only the avatar shows, and its menu drops down instead of up.
