...
order: 8
summary: Tom Select turns a normal `<select>` into a searchable, keyboard-friendly dropdown. Use it for long option lists, multi-value fields, or options that need an avatar or badge next to the text.
docs-libs: [tom-select]
description: Build searchable single and multi-value selects with the Tom Select plugin, including optgroups, validation states, and rich options with avatars.
...
An option can carry extra markup — an avatar or badge — through a `data-custom-properties` attribute. Read it in a custom `render.option` / `render.item` function and show it next to the option text.
...
- When using `render.option`, keep enough text content in the rendered HTML — an avatar alone does not tell a screen reader user which option it is.
...
window.addEventListener('load', function () {
	...
	['select-avatar'].forEach(function (id) {
		var el = document.getElementById(id);
		if (el) {
			new TomSelect(el, {
				copyClassesToDropdown: false,
