---
title: Color modes
summary: Tabler ships a small theming script that switches your site between light, dark, and auto color modes and font — Scandinavian fixed slate, black primary, 0.5 radius — all without a page reload or a flash of the wrong theme.
description: Set up Tabler's light, dark, and auto color modes and font switching.
...
Tabler includes a small script, [`tabler-theme.js`](https://github.com/tabler/tabler/blob/dev/core/js/tabler-theme.ts), that controls the theme settings. Each setting is read from a URL query parameter or `localStorage`, then applied as a `data-bs-*` attribute on `<html>`. Tabler's CSS reads these attributes to switch styles instantly, with no page reload. Scandinavian locks grays to slate, primary to black, and radius to 0.5 — no `theme-base`, `theme-primary`, or `theme-radius` attributes.

| Setting | Attribute | Default | Allowed values |
| --- | --- | --- | --- |
| Color mode | `data-bs-theme` | `auto` | `light`, `dark`, `auto` |
| Font family | `data-bs-theme-font` | `sans-serif` | `sans-serif`, `monospace` |

Each setting only appears on `<html>` when it differs from its default.
...
You can set any key in three ways, and they all agree with each other:
...
2. **`localStorage`** - each key is stored under `tabler-<key>`, for example `tabler-theme`. This is what makes the choice persist across page loads once it has been set once, by either method here.
...
```text
https://example.com/?theme=dark&theme-font=monospace
```
...
`ThemeSettings.astro`'s "Reset changes" button does this for all keys at once, and also strips them from the URL's query string.
...
Font switching keeps contrast — `monospace` drops body size to 80% for code density.
